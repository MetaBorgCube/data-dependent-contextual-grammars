...
public abstract class TokenizedTreeImploder<ParseForest extends AbstractParseForest, ParseNode extends ParseForest, Derivation extends IDerivation<ParseForest>, Tree> implements IImploder<ParseForest, Tree> {
	...
	protected Tree implodeParseNode(ParseNode parseNode, IToken leftToken, IToken rightToken) {
	    ISGLRProduction production = parseNodeProduction(parseNode);
	...
	protected Tree implodeDerivation(Derivation derivation, IToken leftToken, IToken rightToken) {
	    ISGLRProduction production = derivation.production();
	...
	protected void implodeChildParseNodes(List<Tree> childASTs, Derivation derivation, ISGLRProduction production, IToken leftToken, IToken rightToken, List<ParseForest> nonAstLexicals) {
        ...
        for (int i = 0; i < childParseForests.length; i++) {
            ...
            ISGLRProduction parseNodeProduction = parseNodeProduction(parseNode);
    ...
    protected Tree createContextFreeTerm(ISGLRProduction production, List<Tree> childASTs, IToken leftToken, IToken rightToken) {
    ...
    protected Tree createLexicalTerm(ISGLRProduction production, String lexicalString, IToken leftToken, IToken lexicalToken) {
    ...
    protected abstract ISGLRProduction parseNodeProduction(ParseNode parseNode);
